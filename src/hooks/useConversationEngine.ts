    ...
    async (
      ...
      if (phase === "refine") {
          ...
          try {
            ...
            if (allPass) {
              addMessage({
                ...
                options: ["Save my draft 💾", "Generate an image 🖼️", "Preview the post 👁️", "Just copy and go! 🚀"],
              });
            } else {
        ...
        try {
          ...
          addMessage({
            role: "assistant",
            content: `**Here's what I changed:** ✨\n${changeLog}${reduction}\n\nHow does that feel? We can keep iterating or move on when you're happy!\n\n📋 *This edit has been logged to your Edit History — click the 🕐 icon to review all changes.*`,
            options: ["Better! What else can we improve?", "Hmm, not quite right", "Run a full diagnostic", "Looks great — we're done! 🎉"],
          });
        } catch (err) {
          addMessage({
            role: "assistant",
