...
const Index = () => {
  ...
  const actions = {
    ...
    pushDraftHistory, popDraftHistory,
  };
  ...
  // Regenerate drafts when settings change and acknowledge in chat
  ...
  useEffect(() => {
    ...
    if (prev.tone !== session.settings.tone) changes.push(`tone → **${session.settings.tone}**`);
    if (prev.seniority !== session.settings.seniority) changes.push(`audience → **${session.settings.seniority}**`);
    if (prev.depth !== session.settings.depth) changes.push(`depth → **${session.settings.depth}**`);

    pushDraftHistory(`Settings: ${changes.join(", ")}`);
    ...
    if (changes.length > 0) {
      addMessage({
        role: "assistant",
        content: `Settings updated: ${changes.join(", ")}. Drafts regenerated with the new settings.`,
      });
    }
  }, [session.settings]);
